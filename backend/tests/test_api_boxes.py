...
def test_create_list_get(client, auth) -> None:
    b = _create(client, auth, "Box 2", location_text="garage")
    assert (b["label"], b["name"], b["location_text"], b["source"]) == ("2", "Box 2", "garage", "text")
    ...
    assert client.get("/api/boxes/by-label/9", headers=auth).status_code == 404
    assert client.get("/api/boxes/next-label", headers=auth).json() == {"next_label": "3"}


def test_labels_follow_the_users_language(client, auth) -> None:
    client.patch("/api/me", json={"lang": "zh"}, headers=auth)
    b = _create(client, auth, "Box 2")
    assert (b["label"], b["name"]) == ("2号", "2号箱")
    assert client.get("/api/boxes/next-label", headers=auth).json() == {"next_label": "3号"}


def test_duplicate_label_conflict(client, auth) -> None:
    _create(client, auth, "1")
...
def test_patch_label_renormalises_and_checks_conflicts(client, auth) -> None:
    a = _create(client, auth, "1")
    _create(client, auth, "2")
    r = client.patch(f"/api/boxes/{a['id']}", json={"label": "box 5"}, headers=auth)
    assert (r.json()["label"], r.json()["name"]) == ("5", "Box 5")
    r = client.patch(f"/api/boxes/{a['id']}", json={"label": "7", "name": "Camping"}, headers=auth)
    assert (r.json()["label"], r.json()["name"]) == ("7", "Camping")
    assert client.patch(f"/api/boxes/{a['id']}", json={"label": "2"}, headers=auth).status_code == 409
...
def test_items_crud_and_reembed_on_rename(client, auth) -> None:
    b = _create(client, auth, "1")
    r = client.post(f"/api/boxes/{b['id']}/items", json={"name": "tent", "qty_text": "×1"}, headers=auth)
    assert r.status_code == 200 and r.json()["name"] == "tent"
    item = r.json()
    r = client.patch(f"/api/boxes/{b['id']}/items/{item['id']}", json={"name": "2-person tent", "qty_text": "×2"}, headers=auth)
    assert (r.json()["name"], r.json()["qty_text"]) == ("2-person tent", "×2")
    ...
    assert abs(sum(a * b for a, b in zip(vec, fake_vector("2-person tent"), strict=True)) - 1.0) < 1e-4
...
def test_resolve_by_code_label_and_create(client, auth) -> None:
    ...
    r = client.post("/api/boxes/resolve", json={"code": "BX-1", "label": "box 1"}, headers=auth).json()
    ...
    r = client.post("/api/boxes/resolve", json={"code": "BX-9", "label": "1"}, headers=auth).json()
