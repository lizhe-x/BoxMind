...
def _merge_qty(a: str, b: str) -> str:
    ...
    if na and nb:
        ...
        unit = re.sub(r"[×x\d\s]+", "", a or "").strip() or re.sub(r"[×x\d\s]+", "", b or "").strip()
...
def summarize(db: Session, user: User, tool: str, args: dict) -> str:
    if tool == "delete_item":
        return f"从「{args.get('box')}」删除物品「{args.get('item_name')}」"
    if tool == "delete_box":
        box = resolve_box(db, user, args.get("box", ""))
        n = len(box.items) if box else 0
        return f"删除整个箱子「{args.get('box')}」" + (f"(含 {n} 类物品)" if n else "")
    if tool == "empty_box":
        box = resolve_box(db, user, args.get("box", ""))
        n = len(box.items) if box else 0
        return f"清空「{args.get('box')}」的 {n} 类物品(保留箱子本身)"
    if tool == "merge_boxes":
        srcs = "、".join(args.get("source_boxes", []))
        return f"把「{srcs}」的物品并入「{args.get('target_box')}」,并删除清空后的来源箱"
    return f"执行 {tool}"


# ── 快照(撤销用) ────────────────────────────
_LABELS = {"delete_item": "删除物品", "delete_box": "删除箱子", "empty_box": "清空箱子", "merge_boxes": "合并箱子"}
...
def _snapshot_before(db: Session, user: User, tool: str, args: dict) -> None:
    ...
    if snap:
        snap.label, snap.payload = _LABELS.get(tool, tool), payload
    else:
        db.add(UndoSnapshot(user_id=user.id, label=_LABELS.get(tool, tool), payload=payload))
    db.commit()
...
async def restore_last(db: Session, user: User) -> dict:
    ...
    db.delete(snap)
    db.commit()
    return {"ok": True, "summary": f"已撤销「{label}」,数据已还原"}
...
def execute(db: Session, user: User, tool: str, args: dict) -> dict:
    ...
    return {"ok": False, "error": f"未知操作 {tool}"}


def _delete_item(db, user, args):
    box = resolve_box(db, user, args.get("box", ""))
    if not box:
        return {"ok": False, "error": "没找到箱子"}
    ...
    if not item:
        return {"ok": False, "error": f"「{box.label}」里没有「{target}」"}
    name = item.name
    db.delete(item)
    db.commit()
    return {"ok": True, "summary": f"已从「{box.name}」删除「{name}」"}
...
def _delete_box(db, user, args):
    ...
    db.commit()
    return {"ok": True, "summary": f"已删除箱子「{name}」"}
...
def _merge_boxes(db, user, args):
    ...
    for ref in args.get("source_boxes", []):
        ...
        for it in list(src.items):
            if it.name in running:
                ...
                running[it.name].qty_text = _merge_qty(running[it.name].qty_text, it.qty_text)
            else:
    ...
    db.commit()
    return {"ok": True, "summary": f"已把「{'、'.join(merged_from)}」并入「{target.name}」"}
